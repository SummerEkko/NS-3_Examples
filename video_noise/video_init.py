...
class StreamingServer(ns.applications.Application):
    ...
    def __init__(self, node, port=STREAMING_PORT, video_file=None):
        if not video_file:
            raise Exception("未指定视频文件")
        import os
        if not os.path.exists(video_file):
            raise Exception("文件不存在", video_file)
        ...
        # 加载视频文件并将其分块
        self.video_chunks = self.load_video_chunks(video_file)

    def __del__(self):
        # 当对象销毁时，从字典中删除实例条目
        del StreamingServer.socketToInstanceDict[self.m_socket]

    def load_video_chunks(self, video_file):
        # 加载视频文件并将其分块
        video_chunks = []
        with open(video_file, 'rb') as file:
            chunk_size = 1000  # 假设每个视频块的大小为1000字节
            while True:
    ...
    def send_block(self, address):
        ...
        # 获取一个块并发送
        block_to_transmit = self.video_chunks[0]
        self.video_chunks.pop(0)
        # print("发送", sha(block_to_transmit).hexdigest())
        # 将字节流编码为ns-3数据包
        packet_with_stream_block = ns.network.Packet(
            bytearray(block_to_transmit), len(block_to_transmit))
...
class StreamingClient(ns.applications.Application):
    ...
    def __init__(self, node, server_address, port=StreamingServer.STREAMING_PORT, video_file=None):
        if not video_file:
            raise Exception("未指定视频文件")
        import os
        if not os.path.exists(video_file):
        ...
        self.output_video = open("video_out.mp4", "wb")
    ...
    def receive(self):
        ...
        # 从数据包中提取字节流并写入文件
        contents = bytearray(packet.__deref__().GetSize())
        packet.__deref__().CopyData(contents, packet.__deref__().GetSize())
        self.output_video.write(contents)
...
# 创建视频服务器和客户端
server = StreamingServer(nodes.Get(0), video_file="video.mp4")
